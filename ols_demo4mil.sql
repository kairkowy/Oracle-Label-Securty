...
이 예제에서 사용할 DB 유저는 docadmin, docuser와 Profiled user(Uxxxx_nn)를 사용합니다.
...
docuser user는 aplication 유저로써 테이블을 소유할 필요가 없기 때문에 별도의 테이블스페이스를 할당하지 않습니다.
...
1.2.3 Profiled user 생성
Profiled user는 어플리케이션 환경에서는 사용자 관리를 위하여 LDAP을 사용하거나 또는 별도 어플리케이션에서 계정을 만들어 사용합니다. 
이 계정을 통하여 다양한 업무기능에 액세스 할 수 있도록 관리를 합니다. 하지만 실제 DB 테이블에 액세스하는 DB 계정은 다양한 조건에 부합하게 구현되지 않는게 현실입니다. 
바로 이 부분에서 보안 이슈가 발생하고 있고 어플리케이션 운영 효율성으로 인해 묵과되고 있습니다. 
다양한 어플리케이션 사용자들을 테이블의 Row 데이터에 접근하는 것을 통제하기 위해서는 사용자를 명확하게 식별할 수 있어야 하며 이를 통한 접근통제를 해야 합니다. 
이 때 사용할 수 있는 방법으로 오라클은 다양한 섬세한(Fine Grain) 통제 방법을 제공합니다. 
이 데모에서는 사용자 별로 일명 Profiled 계정을 개별로 사용하는 profile access 통제방법을 사용합니다.
기타 proxy user를 포함한 어플리케이션 친화적이면서 효율적인 통제 방법은 오라클 security에서 많은 방법을 제공합니다. 
직접 Administrator 가이드를 참고하거나 오라클 Tech sales consuntant와 상의하십시오.
...
Profiled User는 기본적으로 DB에 접근할 수 있는 privilege만 주고 테이블스페이스는 할당하지 않습니다.
...
----------------------------------------------------------------------------------------------------
DOCUSER
SELECT sa_session.label(policy_name => 'doc_t_pol') session_label FROM dual;
SESSION_LABEL
----------------------------------------------------------------------------------------------------


SELECT sa_session.comp_read(policy_name => 'doc_t_pol') comp_read FROM dual;
COMP_READ
----------------------------------------------------------------------------------------------------
...
sa_session.set_access_profile은 OLS 인증과 권한을 설정하는 프로시저입니다. 
이 프로시저를 실행하기 위해서는 반드시 Profile_Access privelege 가지고 있어야 합니다.
...
EXEC sa_audit_admin.AUDIT(policy_name => 'doc_t_pol', audit_type => 'BY SESSION');

EXEC sa_audit_admin.AUDIT(policy_name => 'doc_t_pol', audit_option => 'PRIVILEGES', audit_type => 'BY SESSION ');
...
select username, userhost, terminal, action, EXTENDED_TIMESTAMP, action_name, comment_text from dba_doc_t_pol_audit_trail;
