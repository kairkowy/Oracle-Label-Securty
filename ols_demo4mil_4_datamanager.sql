...
OLS의 Master administrator의 계정은 LBACSYS이며 일반적인 sysdba 권한을 가진 sys(또는 system)와 별도로 권한이 분리 되어 있습니다. 
OLS Ploicy를 비롯한 관련 Dictionary 정보를 관리합니다. OLS를 처음 사용하는 DB 인스턴스에서는 Account Lock이 된 상태일 수 있습니다. 
다음은 LABCSYS 계정을 활성화합니다.
...
OLS가 활성화 되어 있는지 여부를 확인 합니다. "FALSE" 인 경우에는 활성화 패키지를 실행해야 합니다.

conn sys/Welcome1@pdb17 as sysdba
...
이 예제에서 사용할 DB 유저는 docadmin와 Uxxxx_nn(서비스 유저)등 2종의 유저를 사용합니다.
...
Uxxx_nn 유저는 서비스유저로 명명합니다. 이 예제에서는 비밀관리계정으로 사용됩니다. 이 계정은 docadmin 소유의 비밀데이터가 저장된 테이블의 Row 데이터에 접근할 수 있습니다. 
...
서비스 유저들은 테이블을 소유할 필요가 없기 때문에 별도의 테이블스페이스를 할당하지 않습니다.
...
connect sys/Welcome1@pdb17 as sysdba
...
GRANT EXECUTE on TO_DATA_LABEL TO docadmin;
Grant EXECUTE on LABEL_TO_CHAR TO docadmin;
...
'비밀취급인가수준'의 "2급", "3급"은 해당 비밀등급의 문서를 다룰 수 있는 것을 의미하며, 
"비밀서약자"는 비밀을 다룰 수는 없지만 "대외비" 문서를 조회 할 수 있는 사용자를 의미합니다.
이런 데이터의 단차 속성을 Multi Level Security(MSL) 속성이라고 합니다. 이 예제에서 "소속코드는" OLS Policy(doc_t_policy)에서 그룹명으로 사용됩니다.
...
grant select, update, delete, insert on doc_t to U4000_00,U4200_00 ,U4202_00 ,U4202_01 , 
     U4202_02,U4202_03 , U4204_00, U4204_01, U1000_00, U1200_00, U1202_00, U1202_01, U1205_00, U1205_01;
...
select doc_no,doc_creator,doc_type,doc_state, sign_org,dist_target, UTL_RAW.CAST_TO_VARCHAR2(content) as content 
from docadmin.doc_t;
...
conn docuser/docuser@pdb17
select doc_no,doc_creator,doc_type,doc_state, sign_org,dist_target, UTL_RAW.CAST_TO_VARCHAR2(content) as content from 
docadmin.doc_t;
...
다음 단계를 진행하기 위해 데이터를 삭제합니다.

truncate table doc_t;

2. Oracle Label Security Policy 생성
2.1 Policy 생성
"doc_t_pol"이름의 OLS Policy를 사용하며 Data Label 값이 저장되는 컬럼명은 'doc_t_label'입니다. 
이 Label 컬럼은 OLS administrator에 의해서 관리되는 컬럼이며 사용자에게 보여질수 있고 감출 수도 있습니다(Enable 또는 Disable). 
여기서는 Enabled 모드로 실행합니다. 이 테이블에 적용할 Default Ploicy Enforcement option는 'all_control'입니다. 
이는 Enforce option에서 제공되는 모든 컨트롤(Read Control, Insert Control, Update Control, Delete Control, Label Default, Label Update, Check Control)을 적용한다는 의미입니다. 
enforcement option에 대한 세부내용은 OLS administers guide를 참고하십시오.   

conn lbacsys/Welcome1@pdb17
...
SELECT policy_name, column_name, status, policy_options, policy_subscribed subscribed 
FROM dba_sa_policies where policy_name = 'DOC_T_POL';
...
이번 순서는 "ols_t_pol" policy에서 사용될 OLS 컴포넌트들을 만듭니다. OLS의 모든 policy는 Level,Compartment,Group 이라는 3개의 컴포넌트를 가질 수 있습니다. 
각 Policy에서 Level 컴포넌트는 반드시 지정이 필요하지만 Compartment 또는 Group 은 업무조건에 따라 없을 수 있습니다.
OLS의 각 Policy별로 Level,Compartment,Group은 short name과 long name을 가지며 각 컴포넌트들의 short name을 유니크한 십진수 값(Numeric tag)으로 치환하여 Dictionary 안에서 관리됩니다. 
...
여기서는 군대에서 사용하는 비밀등급을 기준으로 민감도 level을 지정합니다. 
군사자료는 "군사대외비"<"군사3급비밀"<"군사2급비밀" 순으로 민감도가 올라갑니다. 
...
각 Group은 상부 Group을 지정할 수 있여 계층구조를 만들수 있습니다.
...
테이블 owner인 docadmin user에게는 모든 정책을 우회할 수 있는 권한을 부여합니다. 물론 필요에 따라 모든 정책을 우회하지 않고 적절하게 통제도 가능합니다.
...
다음 진행을 위하여 입략 데이터를 롤백합니다.

rollback;


Apply된 policy를 삭제 또는 disable 할 때는 다음과 같이 SQL command line으로 remove package를 실행합니다.

policy column drop을 포함한 policy remove
EXEC sa_policy_admin.remove_table_policy(policy_name => 'doc_t_pol', schema_name => 'docadmin', table_name => 'doc_t', drop_column=> TRUE);
또는 exec sa_sysdba.drop_policy(policy_name => 'doc_t_pol', drop_column=> TRUE);


policy column 유지 한 상태로 policy remove(Disable)
EXEC sa_policy_admin.remove_table_policy(policy_name => 'doc_t_pol', schema_name => 'docadmin', table_name => 'doc_t', drop_column=> FALSE);
...
5.4.1 char_to_label 함수를 이용하여 line by line으로 라벨인 하는 방법
...
5.4.2 Label Function 생성 및 doc_t_pol 정책에 attatch 하기
...
doc_t_pol 정책이 적용된 테이블에는 마치 네트워크 방화벽에서 초기상태 처럼 docadmin user 외에는 어느 누구도 액세스를 할 수 없습니다. 
Row에 라밸링이 된 이후에는 User session label 값과 Data label 값을 기준으로 OLS에 내장된 Access meidiation 알고리즘에 의하여 액세스가 통제됩니다.
다음 작업은 방화벽의 화이트 리스트를 만드는 작업과 같은 작업으로 Profiled user(비밀관리계정)에게 권한을 부여하여 라벨된 데이터에 접근하는 방법을 진행하겠습니다.

7.1 Access Mediation 적용 상태 확인
user session에 라벨 권한을 받지 못한 어떠한 user Session에 대해서도 docadmin.doc_t 테이블에 접근을 허용하지 않습니다. 
만약 비승인 세션이 접근할 경우에는 DB 커널에서 강제로 접근을 차단시키고 아래와 같은 에러 메세지를 리턴할 것입니다.
뿐만 아니라 유저가 세 정보를 변경하여 우회하는 경로도 허용하지 않습니다.

conn u4202_03/Welcome1@pdb17
...
col session_label format a30
col comp_read format a30
...
docadmin.doc_t 테이블 데이터에 대한 액세스는 docuser를 통해서 Profile_Access 프로시져에 의해 허용된 세션만 접근을 허용합니다.
...
conn U4202_00/Welcome1@pdb17

col username format a20
col session_label format a30
col Comp_read format a30
col Group_read format a30
...
SELECT sa_session.sa_user_name(policy_name => 'doc_t_pol') usernme FROM dual;
...
AFOC_OP 소속의 U4202_02 유저 세션은 3급비밀취급인가를 받은 자로서, 3급 비밀을 포함한 하위 비밀을 생산, 관리 할 수 있는 권한을 가지고 있습니다. 
아래 예제는 이런 특징을 반영한 정책을 확인할 수 있는 예제입니다.
...
ORA-12406: unauthorized SQL statement for policy DOC_T_POL
ORA-06512: at "LBACSYS.LBAC_STANDARD", line 20
ORA-06512: at "LBACSYS.LBAC$AU0_73751", line 1
ORA-04088: error during execution of trigger 'LBACSYS.LBAC$AU0_73751'
...
8.3.3 User session에 Compartment, Group에 대한 Read_write 권한 할당
...
8.3.4 상속관계가 없는 타조직에 액세스 권한 부여 방법 및 검증
...
SELECT sa_session.sa_user_name(policy_name => 'doc_t_pol') Ses_user, 
sa_session.label(policy_name => 'doc_t_pol') Sess_Label, 
sa_session.comp_read(policy_name => 'doc_t_pol') Sess_Comp, 
sa_session.group_read(policy_name => 'doc_t_pol') Sess_group 
FROM dual;
...
conn lbacsys/Welcome1@pdb17
/* U1000_00 유저 세션 level 생성 및 권한 부여*/
...
conn U1000_00/Welcome1@pdb17
select doc_no,doc_creator,doc_type,doc_state, sign_org,dist_target, label_to_char(doc_t_label) label, UTL_RAW.CAST_TO_VARCHAR2(content) as content from docadmin.doc_t;
DOC_NO	   DOC_CREATOR		DOC_TYPE   DOC_STATE  SIGN_ORG		   DIST_TARGET			  LABEL 			 CONTENT
---------- -------------------- ---------- ---------- -------------------- ------------------------------ ------------------------------ ------------------------------
20-0001    4202_01              2급        배부       AFOC_OP              GOC_OP                         S:SA:GOC_OP,AFOC_OP            20-0001 문서는 2급 입니다

9. OLS Policy audit 방법
9.1 policy audit

Policy 정보는 중요한 정보에 대한 액세스 통제가 되는 정책입니다. 따라서 Policy 접속, 변경 등  SA_AUDIT_ADMIN 패키지 사용에 대한 모든 작업을 로깅하여 사후 감사나 추적에 사용할 필요가 있습니다. 
오라클 DB에서 제공하는 audit 기능을 사용하여 policy가 적용된 테이블에 대한 audit를 시작합니다.
...
GRANT EXECUTE ON SA_AUDIT_ADMIN TO DOCADMIN;
...
SET serveroutput ON
BEGIN
  IF sa_audit_admin.audit_label_enabled('doc_t_pol')
    THEN dbms_output.put_line('OLS doc_t_pol labels are being audited.');
  ELSE
    dbms_output.put_line('OLS doc_t_pol labels not being audited.');
  END IF;
END;
/
...
select username, owner, timestamp, action, action_name,sessionid,statementid, comment_text, DOC_T_LABEL from lbacsys.dba_doc_t_pol_audit_trail;
...
EXEC sa_sysdba.drop_policy(policy_name => 'doc_t_pol', drop_column => TRUE);
